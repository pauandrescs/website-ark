...
// Memory cache to prevent redundant DB calls during build
let buildPostsCache = null;

export async function getBlogPosts() {
  const isBuild = process.env.NODE_ENV === 'production';
  
  // Use memory cache during build to avoid multiple workers hitting timeouts
  if (isBuild && buildPostsCache !== null) {
    return buildPostsCache;
  }

  const start = Date.now();
  if (isBuild) console.log('>>> [BUILD] getBlogPosts START (Querying DB...)');
  
  try {
    let res;
    if (isBuild) {
      res = await queryBlog("SELECT * FROM posts WHERE status = 'published' ORDER BY date DESC");
      buildPostsCache = res || []; // Store in memory
      console.log(`>>> [BUILD] getBlogPosts END (${Date.now() - start}ms)`);
      return buildPostsCache;
    }

    return await withCache('blog:all_published', 300, async () => {
      return await queryBlog("SELECT * FROM posts WHERE status = 'published' ORDER BY date DESC");
    });
  } catch (err) {
    if (isBuild) {
      console.log(`>>> [BUILD] getBlogPosts ERROR: ${err.message}`);
      buildPostsCache = []; // Prevent further attempts during this build
    }
    return [];
  }
}

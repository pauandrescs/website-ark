...
const getRedisClient = () => {
  ...
  if (!global._redisClient) {
    try {
      ...
      client.on('error', (err) => {
        // Only log the first error to avoid console flooding
        if (!isBuild && !hasLoggedError) {
          console.warn('>>> Redis is offline. Application will use MySQL fallback. (Only logging this once)');
          hasLoggedError = true;
        }
      });
      ...
      client.connect().catch((err) => {
        if (!isBuild && !hasLoggedError) {
          console.warn('>>> Redis connection failed. Using MySQL fallback.');
          hasLoggedError = true;
        }
      });
